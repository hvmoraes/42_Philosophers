...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/03/23 11:13:40 by hcorrea-          #+#    #+#             */
/*   Updated: 2023/03/30 11:01:55 by hcorrea-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
*	subject.
*/
void	write_status(t_philo *philo, bool main_thread_report, t_status status)
{
	pthread_mutex_lock(&philo->data->write_lock);
	if (has_simulation_stopped(philo->data) == true
		&& main_thread_report == false)
	{
...
void	write_outcome(t_data *data)
{
	...
	{
		if (data->philos[i]->times_ate >= (unsigned int)data->eat_count)
			full_count++;
		i++;
	}
	pthread_mutex_lock(&data->write_lock);
	printf("%d/%d philosophers had at least %d meals.\n",
		full_count, data->nbr_philos, data->eat_count);
